...
public class SwipeRefreshView extends SwipeRefreshLayout implements AbsListView.OnScrollListener {
    ...
    /**
     * @param dragScrollListener
     */
    public void setDragScrollListener(OnDragScrollListener dragScrollListener) {
        mDragScrollListener = dragScrollListener;
    }

    @Override
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (null != mDragScrollListener)
            mDragScrollListener.onScrollStateChanged(view, scrollState);
    }

    @Override
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (null != mDragScrollListener)
            mDragScrollListener.onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
        // 滚动时到了最底部也可以加载更多
        if (canLoad()) {
            //loadData();
            Log.i("swipe", "onScroll");
        }
    }
